...
describe("Unit Testing ContractServer", () => {
    ...
    it("should resolve with Format Error when argument list is empty", async () => {
        let response = await ContractServer.invokeMatchingContractToRequest(test.contracts, new ContractServerRequest("read", []));
    ...
    it("should resolve with Format Error when argument rpc is missing", async () => {
        let response = await ContractServer.invokeMatchingContractToRequest(test.contracts, new ContractServerRequest("read", [{ key: "id", value: "abc" }]));
        assert.deepEqual(response, ContractServerResponse.FormatError());
    });

    it("should resolve with Not Found Error when contract cannot be found - cause 'name'", async () => {
        let response = await ContractServer.invokeMatchingContractToRequest(test.contracts, new ContractServerRequest("read", [{ key: "rpc", value: "getdummy0" }]));
    ...
    it("should resolve with Not Found Error when contract cannot be found - cause 'role'", async () => {
        let response = await ContractServer.invokeMatchingContractToRequest(test.contracts, new ContractServerRequest("create", [{ key: "rpc", value: "getdummy1" }]));
        assert.deepEqual(response, ContractServerResponse.NotFoundError());
    });
    ...
    it("should resolve with Format Error when arguments are missing", async () => {
        let response = await ContractServer.invokeMatchingContractToRequest(test.contracts, new ContractServerRequest("read", [{ key: "rpc", value: "getdummy2" }]));
        assert.deepEqual(response, ContractServerResponse.FormatError());
    });

    it("should resolve with success when invoking with correct arguments", async () => {
        let response = await ContractServer.invokeMatchingContractToRequest(test.contracts, new ContractServerRequest("read", [{ key: "rpc", value: "getdummy2" }, { key: "id", value: "abc" }]));
    ...
    it("should resolve with success when invoking with correct arguments + optional arguments", async () => {
        let response = await ContractServer.invokeMatchingContractToRequest(test.contracts, new ContractServerRequest("read", [{ key: "rpc", value: "getdummy2" }, { key: "id", value: "abc" }, { key: "name", value: "bcd" }]));
        assert.deepEqual(response, ContractServerResponse.Success("read", "success"));
    });

    it("should resolve with success when invoking with correct but duplicate arguments", async () => {
        let response = await ContractServer.invokeMatchingContractToRequest(test.contracts, new ContractServerRequest("read", [{ key: "rpc", value: "getdummy2" }, { key: "id", value: "abc" }, { key: "id", value: "bcd" }]));
        assert.deepEqual(response, ContractServerResponse.Success("read", "success"));
    });

    it("should resolve with success when invoking with correct muliple arguments", async () => {
        let response = await ContractServer.invokeMatchingContractToRequest(test.contracts, new ContractServerRequest("read", [{ key: "rpc", value: "getdummy3" }, { key: "id", value: "abc" }, { key: "name", value: "bcd" }]));
        assert.deepEqual(response, ContractServerResponse.Success("read", "success"));
    });
